	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.name == "non-200 response" {
				...
				errorHost := errorURL.Hostname()
				errorPort, err := strconv.Atoi(errorURL.Port())
				if err != nil {
					t.Fatal(err)
				}

				got := checkKafkaRestReady(errorHost, errorPort, tt.timeout, tt.secure, tt.ignoreCert, tt.username, tt.password)
				if got != tt.want {
					t.Errorf("checkKafkaRestReady() = %v, want %v", got, tt.want)
				}
				return
			}

			got := checkKafkaRestReady(tt.host, tt.port, tt.timeout, tt.secure, tt.ignoreCert, tt.username, tt.password)
			if got != tt.want {
				t.Errorf("checkKafkaRestReady() = %v, want %v", got, tt.want)
			}
		})
